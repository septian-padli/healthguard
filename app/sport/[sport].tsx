...
const SportTracker: React.FC = () => {
    ...
    const statusConditionBody = async () => {
        const queryAi = sportType === 'running' ? 
    ...
    useEffect(() => {
        if (bodyCondition.includes('Bahaya')) {
            ...
            router.push(`/emergency/waiting?duration=${encodeURIComponent(formatDuration(sportState.milliseconds))}&steps=${encodeURIComponent(sportState.steps)}&condition=${encodeURIComponent(bodyCondition)}`);
        }
